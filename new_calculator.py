...
def on_button_click(button, btn_widget):
    ...
    elif button == "√":  # Додаємо обробку кореня
        try:
            number = float(current_expression) if current_expression else float(display.get())
            result = math.sqrt(number)  # Обчислюємо корінь
            display.delete(0, tk.END)
            display.insert(0, str(result))
            current_expression = str(result)
